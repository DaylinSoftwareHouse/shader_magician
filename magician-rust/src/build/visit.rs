...
impl <'a> IdentCollector<'a> {
    pub fn new(module_segments: Vec<String>, idx: &'a ProjectIndex) -> Self {
        let builtins = [
        ...
        Self { found: HashSet::new(), module_segments, builtins, idx }
    }

    pub fn collect_from_fn(f: &ItemFn, module_segments: Vec<String>, idx: &'a ProjectIndex) -> HashSet<String> {
        let mut c = Self::new(module_segments, idx);
        c.visit_item_fn(f);
        c.found
    }
    ...
    fn normalise_path(&self, p: &Path) -> Option<String> {
        ...
        let qualified = if let Some(use_path) = self.idx.uses.get(&segments[0]) { use_path.clone() }
            else {
